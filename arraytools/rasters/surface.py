...
:Modified: 2016-12-27
:Purpose:  Calculate slope, aspect, hillshade and other terrain derivatives
...
:
:----------------------
: (3) Slope calculation:   3rd order finite distance Horn (1981) see Burrough
:----------------------
...
:
: Note:  These are all the same... since we are working with a 4d array
...
:   return cell
: or ... azimuth np.mod((450.0 - 45), 360)      => 45  **** best one ****
: or ...         divmod((450.0 - 45), 360)[1]   => (1.0, 45.0)
:
:  [dz/dx] = ((c + 2f + i) - (a + 2d + g)) / 8
...
: (5) Hillshade:
:--------------
(a)  Hillshade = 255.0 * ((cos(Zenith_rad) * cos(Slope_rad)) +
                 (sin(Zenith_rad) * sin(Slope_rad) *
                  cos(Azimuth_rad - Aspect_rad)))
(b)  Zenith_deg = 90 - Altitude
     Convert to radians:
(c)  Zenith_rad = Zenith * pi / 180.0
(d)  Azimuth_math = 360.0 - Azimuth + 90       # normally they are doing this
     Note that if Azimuth_math >= 360.0, then: # to rotate back to the x-axis
(e)  Azimuth_math = Azimuth_math - 360.0       # I skip this and use North
     Convert to radians:
(f)  Azimuth_rad = Azimuth_math * pi / 180.0
Computing slope and aspect
:
:
: (6) Curvature:    Z1  Z2  Z3 - L cell width, L2=L^2, L3=L^3, L4=L^4
:--------------     Z4  Z5  Z6
:                   Z7  Z8  z9
...
:
:-----------------
:interweave arrays:  Various examples: list comprehensions, column_stack,
:-----------------   row_stack, r_, c_, vstack, hstack
:  a = np.arange(5)
:  b = np.arange(5,0,-1)
...
:  http://pangea.stanford.edu/~samuelj/musings/dems-in-python-pt-3-slope
:       -and-hillshades-.html
:  https://github.com/perrygeo/gdal_utils/blob/master/gis-bin/hillshade.py
:  http://matplotlib.org/examples/specialty_plots/
:       topographic_hillshading.html
:  https://blogs.esri.com/esri/arcgis/2015/05/21/
:        take-your-terrain-mapping-to-new-heights/
:  http://gis.stackexchange.com/questions/146296/
:        how-to-create-composite-hillshade
:  https://github.com/Blarghedy/TerrainGen-Python
:  http://vterrain.org/Elevation/Artificial/:
:  *** http://geogratis.gc.ca/site/eng/extraction?
:      id=2016_56ae834dd24892.15336554
:  http://www.jennessent.com/downloads/
:         dem%20surface%20tools%20for%20arcgis.pdf
:
:Requires:
:--------
...
# ---- Demo section ----------------------------------------------------------
#
def __slope_aspect_demo__(cell_size=2):
    ...
    slope = slope_a(data, cell_size=cell_size)
...
def _demo(cell_size=5, degrees=True, pad=False, verbose=True):
    ...
    for d in ds:
        ...
        if pad:
            a = pad_a(a)
        sl = slope_a(a, cell_size, degrees, verbose=False, keepdims=False)
